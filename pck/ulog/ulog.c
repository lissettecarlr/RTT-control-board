...
static void do_output(rt_uint32_t level, const char *tag, rt_bool_t is_raw, const char *log_buf, rt_size_t log_len)
{
    ...
    else
    {
        ...
        ulog_console_backend_output(NULL, level, tag, is_raw, log_buf, log_len);
				...
				extern void ulog_easyflash_backend_output(struct ulog_backend *backend, rt_uint32_t level, const char *tag, rt_bool_t is_raw,
								const char *log, size_t len);
			 ulog_easyflash_backend_output(NULL, level, tag, is_raw, log_buf, log_len);
#endif
...
int ulog_tag_lvl_filter_set(const char *tag, rt_uint32_t level)
{
    ...
    {
        tag_lvl = rt_slist_entry(node, struct ulog_tag_lvl_filter, list); //获取节点列表结构
...
//通过传入标签和等级，进行打印过滤
//eg ulog_tl main 4  //屏蔽main标签4等级以下的打印
static void ulog_filter_tl(int argc, char**argv)
{
	char name[10];
	//uint8_t level;
	if(argc!=3)
	{
		  rt_kprintf("paramter error!ex:[ulog_filter_tl main 4]\n");
		  return;
	}
	//
...
//	level = atoi(&argv[2][0]);
//	rt_kprintf("level :%d\n", level);
	uint8_t tag_len=rt_strlen(&argv[2][0]);
	if(tag_len!=1)
	{
	   rt_kprintf("paramter tag error!ex:[ulog_filter_tl main D]\n");
		 return;
	}
	rt_kprintf("tag :%c\n", argv[2][0]);
	switch (argv[2][0])
	{
	  case 'W':{
		  ulog_tag_lvl_filter_set(name,LOG_LVL_WARNING);
		}break;
		case 'E':{
		  ulog_tag_lvl_filter_set(name,LOG_LVL_ERROR);
		}break;
		case 'I':{
		  ulog_tag_lvl_filter_set(name,LOG_LVL_INFO);
		}break;
		case 'D':{
		  ulog_tag_lvl_filter_set(name,LOG_LVL_DBG);
		}break;
		case 'O':{
		 ulog_tag_lvl_filter_set(name,LOG_FILTER_LVL_ALL);
		}break;
		case 'C':{
		 ulog_tag_lvl_filter_set(name,LOG_FILTER_LVL_SILENT);
		}break;
	}
	rt_kprintf("filter set ok[tag=%s,lvl=%c]\n",name,argv[2][0]);
}

MSH_CMD_EXPORT(ulog_filter_tl, ulog_tag_level_filter);


//通过标签全局过滤，为空则取消过滤
static void ulog_filter_t(int argc, char**argv)
{
	char name[10];
	if(argc ==2)
	{
		rt_memcpy(name,&argv[1][0],rt_strlen(&argv[1][0])+1);
		ulog_global_filter_tag_set(name) ;
	}
	else if(argc ==1)
	{
	   ulog_global_filter_tag_set("");
	...
	{ 
	  rt_kprintf("paramter tag error!ex:[ulog_filter_t main]\n");
	}   
	rt_kprintf("filter set ok[tag=%s]\n",name);
}
MSH_CMD_EXPORT(ulog_filter_t, ulog_filter_tag);


//按照等级屏蔽
static void ulog_filter_l(int argc, char**argv)
{
	if(argc!=2)
	{
	    rt_kprintf("main_log arg error\n");
		  return;
	}
	
	uint8_t tag_len=rt_strlen(&argv[1][0]);
	if(tag_len!=1)
	{
	   rt_kprintf("paramter tag error!ex:[ulog_filter_l D]\n");
		 return;
	}
	rt_kprintf("tag :%c\n", argv[1][0]);
	switch (argv[1][0])
	...
	rt_kprintf("filter set ok[lvl=%c]\n",argv[1][0]);
	
//	level = atoi(&argv[1][0]);
//	rt_kprintf("level :%d\n", level);
//	ulog_global_filter_lvl_set(level);
//  rt_kprintf("ulog level ok\n");
}
MSH_CMD_EXPORT(ulog_filter_l, ulog_filter_level);

//按关键词全局过滤
static void ulog_filter_k(int argc, char**argv)
{
	char name[10];
	if(argc ==2)
	{
		rt_memcpy(name,&argv[1][0],rt_strlen(&argv[1][0])+1);	
		ulog_global_filter_kw_set(name) ;	
	}
	else if(argc ==1)
	{
	  ulog_global_filter_kw_set("");
	...
	{ 
	  rt_kprintf("paramter tag error!ex:[ulog_filter_k key]\n");
		return;
	}   
	rt_kprintf("filter set ok[name=%s]\n",name);
	
}
MSH_CMD_EXPORT(ulog_filter_k, ulog_filter_key);
